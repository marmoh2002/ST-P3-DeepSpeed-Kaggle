...
class FuturePredictionDataset(torch.utils.data.Dataset):
    ...
    def get_indices(self):
        ...
        for index in range(len(self.ixes)):
            ...
            for t in range(self.sequence_length):
                ...
                if (previous_rec is not None) and (
                    rec["scene_token"] != previous_rec["scene_token"]
                ):
                    is_valid_data = False
                    break
    ...
    def get_resizing_and_cropping_parameters(self):
        original_height, original_width = (
            self.cfg.IMAGE.ORIGINAL_HEIGHT,
            self.cfg.IMAGE.ORIGINAL_WIDTH,
        )
        final_height, final_width = self.cfg.IMAGE.FINAL_DIM

        resize_scale = self.cfg.IMAGE.RESIZE_SCALE
        resize_dims = (
            int(original_width * resize_scale),
        ...
        for cam in cameras:
            ...
            world_to_car_egopose = np.vstack(
                [
                    np.hstack(
                        (
                            egopose_rotation.rotation_matrix,
            ...
            lidar_to_sensor = (
                car_egopose_to_sensor @ world_to_car_egopose @ lidar_to_world
            )
            ...
            if self.cfg.LIFT.GT_DEPTH:
                ...
                filename = (
                    os.path.basename(camera_sample["filename"]).split(".")[0] + ".npy"
                )
                depth_file_name = os.path.join(base_root, cam, "npy", filename)
                depth = (
                    torch.from_numpy(np.load(depth_file_name)).unsqueeze(0).unsqueeze(0)
                )
                depth = F.interpolate(
                    depth, scale_factor=self.cfg.IMAGE.RESIZE_SCALE, mode="bilinear"
                )
                depth = depth.squeeze()
                crop = self.augmentation_parameters["crop"]
    ...
    def get_depth_from_lidar(self, lidar_sample, cam_sample):
        points, coloring, im = self.nusc_exp.map_pointcloud_to_image(
            lidar_sample, cam_sample
        )
        tmp_cam = np.zeros(
            (self.cfg.IMAGE.ORIGINAL_HEIGHT, self.cfg.IMAGE.ORIGINAL_WIDTH)
        )
        points = points.astype(np.int)
        tmp_cam[points[1, :], points[0, :]] = coloring
        tmp_cam = torch.from_numpy(tmp_cam).unsqueeze(0).unsqueeze(0)
        tmp_cam = F.interpolate(
            tmp_cam,
            scale_factor=self.cfg.IMAGE.RESIZE_SCALE,
            mode="bilinear",
            align_corners=False,
    ...
    def get_birds_eye_view_label(self, rec, instance_map, in_pred):
        ...
        for annotation_token in rec["anns"]:
            ...
            if (
                self.cfg.DATASET.FILTER_INVISIBLE_VEHICLES
                and int(annotation["visibility_token"]) == 1
                and in_pred is False
            ):
                continue
            ...
            if "vehicle" in annotation["category_name"]:
                ...
                poly_region, z = self._get_poly_region_in_image(
                    annotation, translation, rotation
                )
    ...
    def _get_poly_region_in_image(
        self, instance_annotation, ego_translation, ego_rotation
    ):
        box = Box(
    ...
    def get_label(self, rec, instance_map, in_pred):
        ...
        segmentation = (
            torch.from_numpy(segmentation_np).long().unsqueeze(0).unsqueeze(0)
        )
        ...
        if index < len(self.ixes) - 1:
            ...
            if rec_t0["scene_token"] == rec_t1["scene_token"]:
                egopose_t0 = self.nusc.get(
                    "ego_pose",
                    self.nusc.get("sample_data", rec_t0["data"]["LIDAR_TOP"])[
                        "ego_pose_token"
                    ],
                )
                egopose_t1 = self.nusc.get(
                    "ego_pose",
                    self.nusc.get("sample_data", rec_t1["data"]["LIDAR_TOP"])[
                        "ego_pose_token"
                    ],
                )
    ...
    def voxelize_hd_map(self, rec):
        ...
        hd_features = self.nusc_maps[map_name].get_map_mask(
            box_coords, rot * 180 / np.pi, elements, canvas_size=canvas_size
        )
        ...
        hd_features = torch.transpose(
            hd_features, -2, -1
        )  # (y,x) replace horizontal and vertical coordinates
        return hd_features
    ...
    def hd_traffic_light(self, map_name, center, stretch, dx, bx, canvas_size):
        ...
        tl_token = self.nusc_maps[map_name].get_records_in_patch(
            my_patch, ["traffic_light"], mode="intersect"
        )["traffic_light"]
        polys = []
        for token in tl_token:
            road_token = self.nusc_maps[map_name].get("traffic_light", token)[
    ...
    def __getitem__(self, index):
        ...
        data["target_point"] = torch.tensor([0.0, 0.0])
