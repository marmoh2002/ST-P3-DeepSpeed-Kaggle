...
class Bottleblock(nn.Module):
    def __init__(self, in_channels, out_channels=None):
        ...
        self.layers = nn.Sequential(
            nn.Conv2d(
                in_channels, bottleneck_channels, kernel_size=7, bias=False, padding=3
            ),
            LayerNorm(bottleneck_channels, eps=1e-6, data_format="channels_first"),
            nn.GELU(),
            nn.Conv2d(
                bottleneck_channels, bottleneck_channels, kernel_size=1, bias=False
            ),
            LayerNorm(bottleneck_channels, eps=1e-6, data_format="channels_first"),
            nn.GELU(),
            nn.Conv2d(
                bottleneck_channels, out_channels, kernel_size=3, bias=False, padding=1
            ),
