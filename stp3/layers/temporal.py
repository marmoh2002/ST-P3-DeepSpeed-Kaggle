...
class SpatialGRU(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, gru_bias_init=0.0):
        ...
        self.conv_state_tilde = nn.Conv2d(
            input_size + hidden_size, hidden_size, kernel_size=3, bias=True, padding=1
        )
        self.conv_decoder = nn.Conv2d(
            hidden_size, input_size, kernel_size=1, bias=False
        )
    ...
    def forward(self, x, state=None):
        ...
        rnn_state = (
            torch.zeros(b, self.hidden_size, h, w, device=x.device)
            if state is None
            else state
        )
...
class Dual_GRU(nn.Module):
    def __init__(
        self, in_channels, latent_dim, n_future, mixture=True, gru_bias_init=0.0
    ):
        ...
        self.conv_update_1 = nn.Conv2d(
            input_size + hidden_size, hidden_size, kernel_size=3, bias=True, padding=1
        )
        self.conv_reset_1 = nn.Conv2d(
            input_size + hidden_size, hidden_size, kernel_size=3, bias=True, padding=1
        )
        ...
        self.conv_update_2 = nn.Conv2d(
            hidden_size + hidden_size, hidden_size, kernel_size=3, bias=True, padding=1
        )
        self.conv_reset_2 = nn.Conv2d(
            hidden_size + hidden_size, hidden_size, kernel_size=3, bias=True, padding=1
        )
        ...
        self.conv_decoder_2 = nn.Conv2d(
            hidden_size, hidden_size, kernel_size=3, bias=True, padding=1
        )
    ...
    def forward(self, x, state):
        ...
        assert (
            c == self.input_size
        ), f"feature sizes must match, got input {c} for layer with size {self.input_size}"
...
class Bottleneck3D(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels=None, kernel_size=(2, 3, 3), dilation=(1, 1, 1)
    ):
        super().__init__()
    ...
    def forward(self, *inputs):
        ...
        for f in self.features:
            ...
            x_pool = nn.functional.interpolate(
                x_pool.view(b * t, c, *x_pool.shape[-2:]),
                (h, w),
                mode="bilinear",
                align_corners=False,
            )
...
class TemporalBlock(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels=None, use_pyramid_pooling=False, pool_sizes=None
    ):
        super().__init__()
        ...
        for kernel_size in self.kernels:
            self.convolution_paths.append(
                nn.Sequential(
                    ...
                    CausalConv3d(
                        self.half_channels, self.half_channels, kernel_size=kernel_size
                    ),
                )
            )
        self.convolution_paths.append(
        ...
        if self.use_pyramid_pooling:
            assert (
                pool_sizes is not None
            ), "setting must contain the list of kernel_size, but is None."
