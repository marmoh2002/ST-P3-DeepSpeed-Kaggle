...
class Decoder(nn.Module):
    def __init__(self, in_channels, n_classes, n_present, n_hdmap, predict_gate):
        ...
        if self.predict_pedestrian:
            self.pedestrian_head = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(
                    shared_out_channels, self.n_classes, kernel_size=1, padding=0
                ),
            )
    ...
    def forward(self, x):
        ...
        hdmap_output = (
            self.hdmap_head(x.view(b, s, *x.shape[1:])[:, self.n_present - 1])
            if self.perceive_hdmap
            else None
        )
        instance_center_output = (
            self.instance_center_head(x) if self.predict_instance else None
        )
        instance_offset_output = (
            self.instance_offset_head(x) if self.predict_instance else None
        )
        ...
        return {
            "segmentation": segmentation_output.view(
                b, s, *segmentation_output.shape[1:]
            ),
            "pedestrian": (
                pedestrian_output.view(b, s, *pedestrian_output.shape[1:])
                if pedestrian_output is not None
                else None
            ),
            "hdmap": hdmap_output,
            "instance_center": (
                instance_center_output.view(b, s, *instance_center_output.shape[1:])
                if instance_center_output is not None
                else None
            ),
            "instance_offset": (
                instance_offset_output.view(b, s, *instance_offset_output.shape[1:])
                if instance_offset_output is not None
                else None
            ),
            "instance_flow": (
                instance_future_output.view(b, s, *instance_future_output.shape[1:])
                if instance_future_output is not None
                else None
            ),
            "costvolume": (
                costvolume.view(b, s, *costvolume.shape[1:])
                if costvolume is not None
                else None
            ),
        }
