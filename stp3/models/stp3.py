...
class STP3(nn.Module):
    ...
    def forward(self, image, intrinsics, extrinsics, future_egomotion):
        ...
        if self.n_future > 0:
            ...
            if self.cfg.PROBABILISTIC.ENABLED:
                ...
                future_prediction_input = present_state.new_zeros(
                    b, 1, self.latent_dim, h, w
                )
    ...
    def get_geometry(self, intrinsics, extrinsics):
        ...
        translation = translation.to(dtype=torch.float32)
        ...
        combined_transformation = rotation.matmul(torch.inverse(intrinsics.float()))
        points = (
    ...
    def encoder_forward(self, x, cam_front_index=1):
        ...
        if self.cfg.MODEL.ENCODER.USE_DEPTH_DISTRIBUTION:
            ...
            x = depth_prob.unsqueeze(1) * x.unsqueeze(
                2
    ...
    def projection_to_birds_eye_view(self, x, geometry, future_egomotion):
        ...
        output = torch.zeros(
            (batch, s, c, self.bev_dimension[0], self.bev_dimension[1]),
            dtype=torch.float,
            device=x.device,
        )
        ...
        for b in range(batch):
            ...
            for t in range(s):
                if t != s - 1:
                    ...
                    flow_geo_tmp = rotation_b.matmul(
                        flow_geo_tmp.unsqueeze(-1)
                    ).squeeze(-1)
            ...
            for t in range(s):
                ...
                geometry_b = (
                    flow_geo[t] - (self.bev_start_position - self.bev_resolution / 2.0)
                ) / self.bev_resolution
                geometry_b = geometry_b.view(N, 3).long()
                geometry_b, x_b = voxel_to_pixel(geometry_b, x_b)

                tmp_bev_feature = torch.zeros(
                    (
                        self.bev_dimension[2],
                        self.bev_dimension[0],
                        self.bev_dimension[1],
                ...
                tmp_bev_feature[
                    geometry_b[:, 2], geometry_b[:, 0], geometry_b[:, 1]
                ] = x_b
    ...
    def calculate_birds_eye_view_features(
        self, x, intrinsics, extrinsics, future_egomotion
    ):
        b, s, n, c, h, w = x.shape
        # Reshape
        ...
        cam_front = (
            unpack_sequence_dim(cam_front, b, s)[:, -1]
    ...
    def distribution_forward(self, present_features, min_log_sigma, max_log_sigma):
        ...
        def get_mu_sigma(mu_log_sigma):
            ...
            if self.training:
                gaussian_noise = torch.randn(
                    (b, s, self.latent_dim), device=present_features.device
                )
            else:
                gaussian_noise = torch.zeros(
                    (b, s, self.latent_dim), device=present_features.device
                )
        ...
        if self.cfg.PROBABILISTIC.METHOD == "GAUSSIAN":
            ...
            if self.training:
                bernoulli_noise = torch.randn(
                    (b, self.latent_dim, h, w), device=present_features.device
                )
            else:
                bernoulli_noise = torch.zeros(
                    (b, self.latent_dim, h, w), device=present_features.device
                )
            ...
            mu_log_sigma = self.present_distribution(present_features)
            present_mu1, present_log_sigma1, present_sample1 = get_mu_sigma(
                mu_log_sigma[:, :, : 2 * self.latent_dim]
            )
            ...
            sample = (
                present_sample1 * coefficient[:, :, 0:1]
    ...
    def select_best_traj(self, trajs, cost_volume, lane_divider, semantic_pred, k=1):
        ...
        """
        sm_cost_fc, sm_cost_fo = self.cost_functiozn(
            cost_volume, trajs[:, :, :, :2], semantic_pred, lane_divider
        )
