...
class Planning(nn.Module):
    def __init__(self, cfg, feature_channel, gru_input_size=6, gru_state_size=256):
        ...
        self.reduce_channel = nn.Sequential(
            ...
            Bottleneck(
                int(feature_channel / 2), int(feature_channel / 2), downsample=True
            ),
    ...
    def select(
        ...
        """
        sm_cost_fc, sm_cost_fo = self.cost_function(
            cost_volume,
            trajs[:, :, :, :2],
            semantic_pred,
            lane_divider,
    ...
    def loss(
        ...
        gt_cost_fc, gt_cost_fo = self.cost_function(
            cost_volume,
            gt_trajs[:, :, :, :2],
            semantic_pred,
            lane_divider,
        ...
        L, _ = F.relu(
            F.relu(gt_cost_fo - sm_cost_fo).sum(-1)
            + (gt_cost_fc - sm_cost_fc)
            + self.compute_L2(trajs, gt_trajs).mean(dim=-1)
        ).max(dim=-1)
    ...
    def forward(
        ...
        output_traj = torch.cat(
            [
                output_traj,
                torch.zeros((*output_traj.shape[:-1], 1), device=output_traj.device),
            ],
            dim=-1,
        )
        ...
        if self.training:
            loss = (
                ...
                + (
                    F.smooth_l1_loss(
                        output_traj[:, :, :2], gt_trajs[:, :, :2], reduction="none"
                    )
                    * torch.tensor([10.0, 1.0], device=loss.device)
                ).mean()
            )
