...
class FuturePrediction(nn.Module):
    def __init__(
        ...
        self.dual_grus = Dual_GRU(
            gru_in_channels, in_channels, n_future=n_future, mixture=mixture
        )
        self.res_blocks1 = nn.Sequential(
            *[Block(in_channels) for _ in range(n_res_layers)]
        )
        ...
        for i in range(self.n_spatial_gru):
            ...
            if i < self.n_spatial_gru - 1:
                self.res_blocks.append(
                    nn.Sequential(*[Block(in_channels) for _ in range(n_res_layers)])
                )
            else:
