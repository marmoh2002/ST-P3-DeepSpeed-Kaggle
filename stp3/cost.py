...
class Cost_Function(nn.Module):
    ...
    def forward(
        ...
        headwaycost = torch.clamp(
            self.headwaycost(trajs, semantic_pred, drivable_area), 0, 100
        )
...
class BaseCost(nn.Module):
    ...
    def get_origin_points(self, lambda_=0):
        ...
        pts = np.array(
            [
                [-H / 2.0 + 0.5 - lambda_, W / 2.0 + lambda_],
                [H / 2.0 + 0.5 + lambda_, W / 2.0 + lambda_],
                [H / 2.0 + 0.5 + lambda_, -W / 2.0 - lambda_],
                [-H / 2.0 + 0.5 - lambda_, -W / 2.0 - lambda_],
            ]
    ...
    def compute_area(self, semantic_pred, trajs, ego_velocity=None, _lambda=0):
        ...
        subcost = semantic_pred[
            ii[:, None, None, None], kk[None, None, :, None], rr, cc
        ].sum(dim=-1)
...
class Rule(BaseCost):
    ...
    def forward(self, trajs, drivable_area):
        ...
        dangerous_area = (
            torch.logical_not(drivable_area)
            .float()
            .view(B, 1, W, H)
            .expand(B, n_future, W, H)
        )
...
class SafetyCost(BaseCost):
    def __init__(self, cfg):
        ...
        self.w = nn.Parameter(torch.tensor([1.0, 1.0]), requires_grad=False)
    ...
    def forward(self, trajs, semantic_pred):
        ...
        for i in range(n_future):
            if i == 0:
                ego_velocity[:, :, i] = (
                    torch.sqrt((trajs[:, :, i] ** 2).sum(axis=-1)) / 0.5
                )
            else:
                ego_velocity[:, :, i] = (
                    torch.sqrt(((trajs[:, :, i] - trajs[:, :, i - 1]) ** 2).sum(dim=-1))
                    / 0.5
                )
    ...
    def forward(self, trajs, lane_divider):
        ...
        for i in range(B):
            ...
            if len(index) != 0:
                ...
                distance = torch.sqrt(
                    (((yx_batch - index) * reversed(self.dx)) ** 2).sum(dim=-1)
                )  # (N, n_future, n)
    ...
    def forward(self, trajs):
        ...
        for i in range(n_future):
            if i == 0:
                ...
                lateral_velocity[:, :, i] = (
                    trajs[:, :, i, 0] - trajs[:, :, i - 1, 0]
                ) / 0.5
                longitudinal_velocity[:, :, i] = (
                    trajs[:, :, i, 1] - trajs[:, :, i - 1, 1]
                ) / 0.5
        for i in range(1, n_future):
            lateral_acc[:, :, i] = (
                lateral_velocity[:, :, i] - lateral_velocity[:, :, i - 1]
            ) / 0.5
            longitudinal_acc[:, :, i] = (
                longitudinal_velocity[:, :, i] - longitudinal_velocity[:, :, i - 1]
            ) / 0.5
        ...
        for i in range(n_future):
            if i == 0:
                ego_velocity[:, :, i] = (
                    torch.sqrt((trajs[:, :, i] ** 2).sum(dim=-1)) / 0.5
                )
            else:
                ego_velocity[:, :, i] = (
                    torch.sqrt(((trajs[:, :, i] - trajs[:, :, i - 1]) ** 2).sum(dim=-1))
        ...
        longitudinal_acc = torch.clamp(
            torch.abs(longitudinal_acc) - self.c_lon_acc, 0, 30
        )
