...
class PanopticMetric(Metric):
    def __init__(
        self, n_classes: int, temporally_consistent: bool = True, vehicles_id: int = 1
    ):
        super().__init__()
    ...
    def combine_mask(
        ...
        instance_id_to_class[torch.arange(n_classes, device=segmentation.device)] = (
...
class PlanningMetric(Metric):
    def __init__(self, cfg, n_future=4):
        super().__init__()
    ...
    def evaluate_coll(self, trajs, gt_trajs, segmentation):
        ...
        device = trajs.device # Get the device from trajs
        ...
        for i in range(B):
            ...
            ti = torch.arange(n_future, device=device) # Move ti to the GPU
