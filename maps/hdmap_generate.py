...
def get_warp_transform(ev_loc, ev_rot, world_offset):
    ...
    bottom_left = (
        ev_loc_in_px - pixels_ev_to_bottom * forward_vec - (0.5 * width) * right_vec
    )
    top_left = (
        ev_loc_in_px
        + (width - pixels_ev_to_bottom) * forward_vec
        - (0.5 * width) * right_vec
    )
    top_right = (
        ev_loc_in_px
    ...
    dst_pts = np.array([[0, width - 1], [0, 0], [width - 1, 0]], dtype=np.float32)
...
for town in tqdm.tqdm(town_list):
    ...
    with h5py.File(maps_h5_path, "r", libver="latest", swmr=True) as hf:
        ...
        lane_marking_yellow_broken = np.array(
            hf["lane_marking_yellow_broken"], dtype=np.uint8
        )
        lane_marking_yellow_solid = np.array(
            hf["lane_marking_yellow_solid"], dtype=np.uint8
        )
        lane_marking_white_broken = np.array(
            hf["lane_marking_white_broken"], dtype=np.uint8
        )
        lane_marking_white_solid = np.array(
            hf["lane_marking_white_solid"], dtype=np.uint8
        )
    ...
    route_list = [
        route
        for route in os.listdir(town_folder)
        if os.path.isdir(os.path.join(town_folder, route))
    ]
    ...
    for route in tqdm.tqdm(route_list):
        ...
        for measurement in measurement_files:
            with open(os.path.join(measurement_folder, measurement), "r") as read_file:
                ...
                lane_mask_white_broken = cv.warpAffine(
                    lane_marking_white_broken, M_warp, (width, width)
                ).astype(np.bool)
                lane_mask_white_solid = cv.warpAffine(
                    lane_marking_white_solid, M_warp, (width, width)
                ).astype(np.bool)
                lane_mask_yellow_broken = cv.warpAffine(
                    lane_marking_yellow_broken, M_warp, (width, width)
                ).astype(np.bool)
                lane_mask_yellow_solid = cv.warpAffine(
                    lane_marking_yellow_solid, M_warp, (width, width)
                ).astype(np.bool)
                ...
                cv.imwrite(
                    os.path.join(
                        route_folder, "hdmap", measurement.replace("json", "png")
                    ),
                    image,
                )
